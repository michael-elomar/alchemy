...
HOST_AUTOTOOLS_CONFIGURE_ENV := \
	...
	LD="$(HOST_LD)" \
	NM="$(HOST_NM)" \
	CC="$(strip $(CCACHE) $(HOST_CC))" \
	GCC="$(strip $(CCACHE) $(HOST_CC))" \
	CXX="$(strip $(CCACHE) $(HOST_CXX))" \
	CPP="$(HOST_CPP)" \
	CXXCPP="$(HOST_CPP)" \
	FC="$(HOST_FC)" \
	RANLIB="$(HOST_RANLIB)" \
...
TARGET_AUTOTOOLS_CONFIGURE_ENV := \
	...
	LD="$(TARGET_LD)" \
	NM="$(TARGET_NM)" \
	CC="$(strip $(CCACHE) $(TARGET_CC))" \
	GCC="$(strip $(CCACHE) $(TARGET_CC))" \
	CXX="$(strip $(CCACHE) $(TARGET_CXX))" \
	CPP="$(TARGET_CPP)" \
	CXXCPP="$(TARGET_CPP)" \
	FC="$(TARGET_FC)" \
