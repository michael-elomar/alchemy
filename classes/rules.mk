...
# don't include prerequisite generated by this module (stamps, exported prerequisites, ...)
_module_all_copy_files_src_prerequisites := \
	$(filter-out  $(_module_all_copy_files_src) $(_module_all_copy_files_dst), \
		$(LOCAL_PREREQUISITES) $(imported_PREREQUISITES))
$(foreach __src,$(_module_all_copy_files_src), \
	$(if $(filter $(__src),$(all_prerequisites)),$(empty), \
		$(eval $(__src): | $(_module_all_copy_files_src_prerequisites)) \
	) \
)
# Also make sure bootstrap is done before destination files
_module_all_copy_files_dst_prerequisites := \
	$(_module_all_copy_files_src_prerequisites) \
	$(_module_bootstrapped_stamp_file)
$(foreach __dst,$(_module_all_copy_files_dst), \
	$(if $(filter $(__dst),$(all_prerequisites)),$(empty), \
		$(eval $(__dst): | $(_module_all_copy_files_dst_prerequisites)) \
	) \
)
